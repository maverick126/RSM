...
namespace HtmlRichText
{
	...
	{ 
		...
		public string GetHTML(bool bHTML, bool bParaFormat)
		{
			...
			try
			{
					...
					if (bHTML)
					{
						...
						if ((strFont != strfname) || (crFont != cf.crTextColor) || (yHeight != cf.yHeight))
						{
							if (strFont != "")
							{
                                ...
                                if (string.Equals(displayText.Substring(i-7, 7).ToLower(), "$token$"))
                                    mfr.strValue = "</table>";
                                else
                                    mfr.strValue = "</font>";
                                colFormat.Add(mfr);
                            ...
                            int fsize = (int) cf.yHeight/20;//cf.cbSize / 6; // (20 * 5)
                            //-------------------------
                            ...
                            int ioffset = 0;
                            while ((i+ioffset) < displayText.Length)
                            {
                                if (displayText.Substring((i + ioffset), 1) != " ")
                                    break;
                                ioffset += 1;
                            }
                            if (displayText.Length > (i + ioffset + 7) && displayText.Substring(i + ioffset, 7).Trim().ToLower().Contains("$token$"))
                                mfr.strValue = "<table face=\"" + strFont + "\" color=\"" + strcolor + "\" style=\"font-size:" + fsize + "pt;\">";
                            else
                                mfr.strValue = "<font face=\"" + strFont + "\" color=\"" + strcolor + "\" style=\"font-size:" + fsize + "pt;\">";
				...
				for (i = 0; i < k; i++)
				{
                    ...
                    { 
                        ...
                        if (strT.Length > mfr.nPos)
                            strHTML += strT.Substring(nAcum, mfr.nPos - nAcum) + mfr.strValue;
                        else if (strT.Length > (nAcum + (mfr.nPos - nAcum)))
                            strHTML += strT.Substring(nAcum) + mfr.strValue;
            ...
            // add style tags
            if (!strHTML.Contains("p { margin: 2px; }"))
                strHTML = "<style>p { margin: 2px; }</style>" + strHTML;
            //strHTML =  strHTML.Replace("p { margin: 2px; }", string.Empty);

            strHTML = strHTML + "</SECTION_END>";
		...
		public void AddHTML(string strHTML)
		{
            ...
            int startIndex = 0;
            int nStart = strHTML.IndexOf("<style>");
            int nEnd = 0;
            string strData = strHTML;
            string strDataAdditional = string.Empty;

            if (nStart > 0)
            {
                nEnd = strHTML.IndexOf("</style>", nStart) + ("</style>").Length;
                strHTML = strHTML.Replace(strHTML.Substring(nStart, nEnd), string.Empty);
            }
            strHTML = strHTML.Replace("<p align=\"left\">", string.Empty);
            strHTML = strHTML.Replace("</font>", string.Empty);
            ...
            try 
			{
				...
				while (strHTML.Length > 0)
				{
                    try { 
					strData = strHTML;
                    strDataAdditional = string.Empty;

                    reinit:
					
					// looking for start tags
                    if (!string.IsNullOrWhiteSpace(strHTML) && strHTML.Length >= startIndex)
					    nStart = strHTML.IndexOf('<', startIndex);
					if (nStart >= 0)
					{
						if (nStart > 0)
						{
							...
							if (nEnd > nStart)
							{
								if ((nEnd - nStart) > 0)
								{
									...
									else if ((strTag.Length > 5) && (strTag.Substring(0, 5) == "<font"))
									{
                                        ...
                                        nSize = strTag.IndexOf("font-size:");
                                        if (nSize > 0)
                                        {
                                            int nSizeEnd = strTag.IndexOf('\"', nSize + 12);
                                            if (nSizeEnd > nSize)
                                            {
                                                yHeight = int.Parse(strTag.Substring(nSize + 10, nSizeEnd - nSize - 10 - 3)); // px; (-3)
                                                yHeight *= 20; // (20 * 5);
                                            }
                                        }
                                    ...
                                    else if (strTag == "<li>")
                                    {
                                        ...
                                        if (pf.wNumbering != PFN_BULLET)
                                        {
                                            ...
                                            pf.wNumbering = (short)0;// PFN_BULLET;
                                            pf.wNumbering = (short)3;
                                        }
                                    }
                                    else if (strTag == "<ol>" || strTag == "</ol>" || strTag == "</li>")
                                    {
                                        startIndex += strTag.Length;
                                        continue;
                                    }
                                    else if (strTag == "<text>" || strTag == "</text>" || strTag == "<section1>" || strTag == "</section_end>" || strTag == "<section2>" || strTag == "</section_end>" || strTag == "<section3>" || strTag == "</section_end>")
                                    {
                                        startIndex += strTag.Length;
                                        continue;
                                    }
                                    else if (strTag == "<space8>" || strTag == "<space4>" || strTag == "<space2>" || strTag == "<space1>")
                                    {
                                        //startIndex += strTag.Length;
                                        strDataAdditional = strTag.ToUpper();
                                    }                                    
									...
									if (nStart2 > 0)
									{
										...
										strData = strDataAdditional + strHTML.Substring(nEnd + 1, nStart2 - nEnd - 1);
									...
									else
									{
										...
										if ((nEnd + 1) < strHTML.Length)
											strData = strDataAdditional + strHTML.Substring(nEnd + 1);
										else
											strData = "";
									...
									if (strData.Length > 0)
									{
										...
										if (string.IsNullOrEmpty(strDataAdditional) && strData[0] == '<')
											goto reinit;
									}
									//-------------------------------
								}
