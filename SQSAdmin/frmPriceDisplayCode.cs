...
namespace SQSAdmin
{
    public partial class frmPriceDisplayCode : Form
    {
        public frmPriceDisplayCode()
        {
            InitializeComponent();
        }
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                codeID = Int32.Parse(dataGridView1.Rows[e.RowIndex].Cells["PriceDisplayCodeID"].Value.ToString());
                ...
                code = dataGridView1.Rows[e.RowIndex].Cells["PriceDisplayCode"].Value.ToString();
                desc = dataGridView1.Rows[e.RowIndex].Cells["PriceDisplayDesc"].Value.ToString();


                DataSet dsTemp = MetriconCommon.DatabaseManager.ExecuteSQLQuery("[spa_AdminAddEditPriceDisplayCode]", new System.Data.SqlClient.SqlParameter[6]
                            {
        ...
        private void frmPriceDisplayCode_Load(object sender, EventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    DataSet dsTemp1 = MetriconCommon.DatabaseManager.ExecuteSQLQuery("select * from tblPriceDisplayCode where PriceDisplayCode='" + txtCode.Text.ToString().Trim() + "'");
                    if (dsTemp1.Tables[0].Rows.Count > 0)
                    {
                        MessageBox.Show("This price display code already exists !");
                    }
                    else
                    {
                        DataSet dsTemp = MetriconCommon.DatabaseManager.ExecuteSQLQuery("[spa_AdminAddEditPriceDisplayCode]", new System.Data.SqlClient.SqlParameter[6]
