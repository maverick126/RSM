...
namespace SQSAdmin
{
    public partial class frmUOM : Form
    {
        ...
        private void loadList()
        {
            ...
            try
            {
                DataSet dsTemp = MetriconCommon.DatabaseManager.ExecuteSQLQuery("[spa_AdminGetUOMCode]", new System.Data.SqlClient.SqlParameter[3]
                                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter a valid UOM code!");
            }
            else
            {
                ...
                try
                {
                    DataSet dsTemp1 = MetriconCommon.DatabaseManager.ExecuteSQLQuery("select * from tblUOM where Code='" + txtCode.Text.ToString().Trim() + "'");
                    if (dsTemp1.Tables[0].Rows.Count > 0)
                    {
                        MessageBox.Show("This UOM code already exists !");
                    }
                    else
                    {
                        DataSet dsTemp = MetriconCommon.DatabaseManager.ExecuteSQLQuery("[spa_AdminAddEditUOMCode]", new System.Data.SqlClient.SqlParameter[6]
                                {
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                codeID = Int32.Parse(dataGridView1.Rows[e.RowIndex].Cells["UOMID"].Value.ToString());
